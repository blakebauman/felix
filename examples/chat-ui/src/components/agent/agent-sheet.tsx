            ...
            {resolved && spec && (
              <>
                <div className="flex flex-wrap items-center gap-1.5">
                  ...
                  {meta?.version && <span className="text-muted-foreground">spec {meta.version}</span>}
                </div>
                ...
                <Section title="Tools & skills">
                  ...
                  <Chips
                    label="skills"
                    items={asArray(spec.skills).map((s) => (s as { name?: string })?.name ?? String(s))}
                  />
                </Section>
                ...
                {(asArray(spec.guardrails?.judges).length > 0 ||
                  ...
                  <Section title="Governance">
                    ...
                    {asArray(spec.approvals).map((a, i) => {
                      ...
                      return <Row key={`appr-${i}`} label={`approval: ${ap.id ?? i}`} value={asArray(ap.tools).join(', ')} />;
                    })}
                ...
                <Section title="Inbound auth">
                  <Row label="anonymous" value={spec.auth?.inbound?.allow_anonymous ? 'allowed' : 'denied'} />
                  {asArray(spec.auth?.inbound?.required_scopes).length > 0 && (
                    <Chips label="scopes" items={asArray(spec.auth?.inbound?.required_scopes).map(String)} />
                  )}
                </Section>
              </>
            )}
...
interface ManifestLike {
  ...
  spec?: {
    ...
    auth?: { inbound?: { allow_anonymous?: boolean; required_scopes?: string[]; schemes?: string[] } };
...
function modelField(model: Record<string, unknown> | undefined, key: string): string | number | undefined {
  const v = model?.[key];
