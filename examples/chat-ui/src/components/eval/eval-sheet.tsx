      ...
      <SheetContent side="right" className="w-full gap-0 p-0 sm:max-w-xl">
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-3 p-4">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {datasets.length === 0 && (
              <span className="text-xs text-muted-foreground">No datasets yet — create one below.</span>
            )}
          </div>
          <div className="flex gap-2">
            ...
            />
            <Button size="sm" className="h-8 gap-1" disabled={creating || !newName.trim()} onClick={create}>
              <PlusIcon className="size-3.5" /> New
            </Button>
          </div>

          {selected ? (
            <DatasetPanel key={selected} dataset={selected} manifest={manifest} onError={setError} />
          ) : null}
        </div>
      </SheetContent>
    </Sheet>
  ...
  return (
    <div className="rounded-md border bg-background p-2.5 text-xs">
      <div className="flex items-center gap-2">
        ...
        <span className="font-mono text-[10px] text-muted-foreground">{run.candidate_manifest}</span>
            ...
            <li key={s.item_id} className="flex items-start gap-2">
              <span
                className={cn(
                  ...
                  s.verdict === 'pass' ? 'text-emerald-600 dark:text-emerald-400' : 'text-destructive',
                )}
              >
                {s.verdict}
              </span>
...
function Heading({ children }: { children: React.ReactNode }) {
  return (
    <div className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">{children}</div>
  );
}
