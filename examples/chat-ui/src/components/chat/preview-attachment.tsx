  ...
  return (
    ...
    >
      {contentType?.startsWith("image") ? (
        // biome-ignore lint/performance/noImgElement: blob/data URL preview
        <img alt={name ?? "attachment"} className="size-full object-cover" src={url} />
      ) : (
      ...
      {onRemove && !isUploading && (
        <button
          ...
          type="button"
        >
          <CrossSmallIcon className="size-2.5" />
        </button>
      )}
    </div>
  );
