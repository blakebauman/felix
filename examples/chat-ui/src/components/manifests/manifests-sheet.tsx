      ...
      <SheetContent side="right" className="w-full gap-0 p-0 sm:max-w-xl">
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-3 p-4">
          ...
          <div className="flex gap-2">
            ...
            />
            <Button size="sm" className="h-8 whitespace-nowrap" disabled={busy || !importName.trim()} onClick={importManifest}>
              Import as version
            </Button>
          </div>

          {selectedRow ? (
            <VersionsPanel key={selectedRow.name} summary={selectedRow} onChanged={refresh} onError={setError} />
          ) : null}
        </div>
      </SheetContent>
    </Sheet>
...
function VersionsPanel({
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-3">
      ...
      {/* Version log + editor */}
      <div className="flex items-center gap-2">
        <span className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Versions</span>
          ...
          {list?.versions.map((v) => (
            <div key={v.version} className="rounded-md border bg-background px-2.5 py-1.5 text-xs">
              <div className="flex items-center gap-2">
                ...
                <span className={cn('truncate text-[10px] text-muted-foreground', !v.comment && 'italic')}>
