...
export async function listAudit(opts: { status?: string; limit?: number } = {}): Promise<AuditEvent[]> {
...
export async function listApprovals(status: ApprovalRequest['status'] = 'pending'): Promise<ApprovalRequest[]> {
 ...
 */
export async function getToolMetrics(opts: { sinceMs?: number; limit?: number } = {}): Promise<ToolMetrics> {
  const q = new URLSearchParams();
...
export async function runEvalDataset(
  ...
  return evalFetch<EvalRunSummary>(`/datasets/${encodeURIComponent(dataset)}/run`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ candidate_manifest: candidateManifest, deterministic_judge: deterministicJudge }),
  });
}
...
export async function getResolvedManifest(name: string, version?: number): Promise<ResolvedManifest> {
...
export async function activateManifestVersion(name: string, version: number): Promise<ManifestPointer> {
 ...
 */
export async function rollbackManifestCanary(name: string, clearVersion = false): Promise<ManifestPointer> {
