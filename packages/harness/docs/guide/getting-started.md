...
```bash
pnpm build:manifests   # YAML manifests + SKILL.md -> packages/harness/src/{manifests,skills}/bundled.ts
pnpm migrate:local     # apply D1 schema (apps/api/migrations/) to local SQLite
pnpm dev               # wrangler dev on http://localhost:8787 (runs in apps/api)
```

`pnpm dev` reruns `build:manifests` first. It bundles the harness package's own `packages/harness/manifests/*.yaml` and `packages/harness/skills/*/SKILL.md`; if those directories are empty the bundle is empty — tests in this repo stub manifests directly through `parseManifest` and `_clearManifestCaches`.
