...
export async function runDataset(
  ...
  const resolved = await resolveManifest(env, opts.tenantId, opts.candidateManifest);
  if (!resolved) {
    await finalizeRun(env, opts.tenantId, opts.runId, { status: 'failed', scores: [] });
    throw new Error(`candidate manifest '${opts.candidateManifest}' not found for this tenant`);
  }
  ...
  await finalizeRun(env, opts.tenantId, opts.runId, { status: 'completed', scores });
