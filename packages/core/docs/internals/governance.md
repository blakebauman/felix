...
`disposeLimitState` in [src/context.ts](../../src/context.ts) is the cleanup hook — clears the timer, fires abort if it hasn't already. Auth middleware calls it in `finally`; cron + queue handlers do the same in [src/index.ts](../../src/index.ts).
