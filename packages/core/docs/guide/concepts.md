...
1. **Built-ins** — registered in `compose(env)` (`src/composition.ts`). The core set is `calculator`, `list_skills`, `activate_skill`, `deactivate_skill`, plus the commerce suite: catalog/cart/order tools, `commerce_checkout`, `commerce_record_consent`, personalization (`recommend_products`, `identify_customer`), visual search (`search_by_image`), and the B2B quote-to-cash tools — see [internals/commerce.md](../internals/commerce.md#tool-catalog) for the full list.
