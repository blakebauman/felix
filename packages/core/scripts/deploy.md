...
# Vectorize (768 dims = bge-base-en-v1.5; matches src/memory/store.ts)
...
```bash
pnpm deploy:staging      # runs build:manifests + build:docs, then wrangler deploy --env staging
```
...
```bash
# Sign your bundle.json (see src/policy/bundle.ts for the canonical-JSON
...
(see [docs/guide/deploy.md](../docs/guide/deploy.md#secrets)).
