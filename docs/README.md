...
- `GET /openapi.json` — OpenAPI 3.1.0 specification covering the core routes (OpenAI surface, `/chat`, `/a2a`, `/mcp`, and the `/audit` · `/approvals` · `/plans` · `/jobs` · `/manifests` · `/eval` management surfaces), grouped by tag; internal back-channel routes are excluded, and the commerce routers are documented in [internals/commerce.md](internals/commerce.md) rather than the spec. A completion-gate test keeps the covered surfaces exhaustive.
- `GET /docs` — Scalar API reference UI rendered from the spec above
- `GET /docs/home` — the prose docs below, rendered as a browsable site (`/docs/guide/*`, `/docs/internals/*`) with a Guide/Internals sidebar. The Markdown in this directory is bundled to HTML at build time (`pnpm build:docs`) and served by the Worker, so the OpenAPI "Read more" links resolve on the deployment rather than off to GitHub.
...
| Build on the commerce layer (shopping, ACP, storefronts, B2B, GEO) | [internals/commerce.md](internals/commerce.md) |
...
docs/
  ...
  internals/
    ...
    commerce.md              Felix Commerce: shopping tools, ACP, storefronts, B2B, GEO, consent
