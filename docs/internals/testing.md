...
|---|---|---|---|
| `unit` | node | `tests/unit/**/*.test.ts` | none — pure logic, schema, governance wrappers, fake DOs |
| `workers` | `@cloudflare/vitest-pool-workers` (miniflare/workerd) | `tests/integration/**/*.test.ts` | declared explicitly in the config (not read from `wrangler.jsonc`) |
...
pnpm test -- tests/unit/manifest_schema.test.ts    # single file
...
1. `wrangler.jsonc` for production.
...
The integration tests boot the worker via `SELF.fetch`; the miniflare entry needs `main: './src/index.ts'` to know where to compile from.
...
### Workers project — `tests/integration/**`
...
5. **Tenant isolation.** Always write at least one assertion proving a different tenant cannot see the rows you just wrote. Composite-key invariants are easy to break in a follow-up; the test is the safety net. `tests/integration/cross_tenant.test.ts` is the dedicated cross-tenant probe and the right reference when adding similar coverage.
