...
From `wrangler.jsonc` — your gitignored copy of the tracked template `wrangler.example.jsonc` (`cp wrangler.example.jsonc wrangler.jsonc`, then fill in the `REPLACE_*` account/resource ids). The dev/local set is the top-level block; `env.staging` and `env.production` override D1 / KV / R2 / Vectorize / Queue with isolated resources.
...
| `CONSUMER_SHARED_SECRET` | when any manifest declares `spec.queues[]` or entity push is used | Shared secret on `POST /internal/sessions/:thread_id/events` and `POST /entities/:type/push`. Consumers send it as `x-consumer-secret`. Generate with `openssl rand -base64 32` and put it on **both** the Felix Worker and every consumer Worker. Missing on Felix → the route returns 503 (refuses to authenticate anyone). |
...
`wrangler dev` reads `.dev.vars` (gitignored). Copy `.dev.vars.example` to `.dev.vars` and fill in. None of these propagate to staging or production — those envs are populated only via `wrangler secret put`.
...
Every 10 minutes the `scheduled` handler runs these in `waitUntil`, each guarded so one failure doesn't starve the rest (`src/index.ts:scheduled`):
