...
List of tool names registered with the `ToolProvider`. The core built-ins are `calculator`, `list_skills`, `activate_skill`, `deactivate_skill`, plus the commerce suite registered in `src/composition.ts`: catalog/cart/order tools (`catalog_search`, `catalog_get`, `catalog_categories`, `cart_view`, `cart_add`, `cart_update`, `cart_remove`, `order_status`), `commerce_checkout`, `commerce_record_consent`, personalization (`recommend_products`, `identify_customer`), visual search (`search_by_image`), and the B2B suite (`account_get`, `buyer_get`, `purchase_authority_check`, `price_lookup`, `create_quote`, `quote_get`, `send_quote`, `accept_quote`, `convert_quote`, `invoice_get`, `pay_invoice`) — see [Agentic commerce](../internals/commerce.md). Skills can fold additional tool names into this list at build time.
