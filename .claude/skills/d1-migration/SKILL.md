...
when_to_use: 'Requests like "new migration", "add a table", "alter table"; D1 schema changes, apps/api/migrations/*.sql edits, tenant isolation, composite primary keys.'
---
...
- **File**: `apps/api/migrations/NNNN_<slug>.sql`, zero-padded 4-digit, next sequential number — always check `ls apps/api/migrations/` first (do not trust a remembered head).
...
- Integration tests apply migrations via `apps/api/tests/integration/setup.ts` automatically; if the table's queries need a new binding, remember `vitest.config.ts` `miniflare.bindings`.
