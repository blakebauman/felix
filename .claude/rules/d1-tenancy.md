...
- Migrations: `apps/api/migrations/NNNN_slug.sql`, next sequential number (check `ls apps/api/migrations/`), plugin-prefixed names for plugin tables; run `pnpm migrate:local` after creating. New tables need `apps/api/tests/integration/cross_tenant.test.ts` coverage and a `packages/harness/docs/internals/persistence.md` + CLAUDE.md persistence-layout update.
